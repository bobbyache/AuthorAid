...
namespace Domain.Positioning
{
    ...
    public sealed class NonMatchingOrdinalReferenceException : ApplicationException, ISerializable
    {
        ...
        public NonMatchingOrdinalReferenceException(string message) : base(message)
        {
            // Add implementation.
        }
        public NonMatchingOrdinalReferenceException(string message, Exception inner) : base(message, inner)
        {
            // Add implementation.
        }

        // This constructor is needed for serialization.
        protected NonMatchingOrdinalReferenceException(SerializationInfo info, StreamingContext context) : base(info, context)
       {
            // Add implementation.
       }
    }
    ...
    public class PositionableList<T> where T : class, IPositionedItem
    {
        ...
        private OrdinalObservableCollection<T> positionedItemList = new OrdinalObservableCollection<T>();
        ...
        public List<T> ItemsList
        {
            ...
            {
                this.OrderAll();
                return positionedItemList.ToList(); 
            }
        }

        public ObservableCollection<T> ObservableItemsList
        {
            get
            {
                this.OrderAll();
                return positionedItemList as ObservableCollection<T>;
            }
        }
        ...
        public T FirstItem
        {
            get { return positionedItemList[0]; }
        }
        ...
        public T ItemAbove(T item)
        {
            if (CanMoveUp(item))
            {
                T above = (from ts in positionedItemList
        ...
        public T ItemBelow(T item)
        {
            if (CanMoveDown(item))
            {
                T below = (from ts in positionedItemList
                           where ts.Ordinal == item.Ordinal + 1
                           select ts).SingleOrDefault();
                return below;
            }
        ...
        {
            positionedItemList = new OrdinalObservableCollection<T>(itemList);
            AutoFixPositioning();
            OrderAll();
        }
